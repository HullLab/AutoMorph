function batch2dmorph(directory,image_extension,sampleID,output_filename,microns_per_pixel,get_coordinates,save_intermediates,intensity_range_in,intensity_range_out,gamma,threshold_adjustment,smoothing_sigma,write_csv,downsample,num_points)
%Output:
%   
%   FILES:
...
for file = files'
    ...
    % Extract 2D outline
    [obj_final,obj_edge,obj_smooth,sampleID,objectID] = extract2doutline(file.name,sampleID,save_intermediates,intensity_range_in,intensity_range_out,gamma,threshold_adjustment,smoothing_sigma); 
    ...
    if ~(length(temp_perimeter_bumpy) == 1) || ~(length(temp_perimeter_smooth) == 1) || max(obj_edge(:)) == 0
    ...
    temp_table = struct2table(measuremorph(obj_final,obj_edge,obj_smooth,sampleID,objectID,microns_per_pixel,write_csv));
...
% Write final output files
    ...
    % Write file containing names of skipped images, and copy images to
