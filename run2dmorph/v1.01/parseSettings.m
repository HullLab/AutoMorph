...
while ischar(textline)
	if ~isempty(textline)			% If we're not a blank line...
		if textline(1) ~= '#'			%  .. and we're not a comment..
			...
			elseif strcmp(variable, 'microns_per_pixel') == 1
				% Convert to double and assign:
                if strcmp(options{1},'[]') == 1
                    microns_per_pixel = [];
                else
                    microns_per_pixel = str2double(options{1});
                end
...
settings = repmat(struct('directory', directory, 'image_extension', image_extension, 'sampleID', sampleID, 'output_filename', output_filename, 'microns_per_pixel', microns_per_pixel, 'get_coordinates', get_coordinates, ...
									       'save_intermediates', save_intermediates, 'intensity_range_in', intensity_range_in, 'intensity_range_out', intensity_range_out, 'gamma', gamma, 'threshold_adjustment', threshold_adjustment, 'smoothing_sigma', smoothing_sigma, 'write_csv',write_csv,'downsample',downsample,'num_points',num_points), 1, 1);
end
