...
if not skipMeshExtraction:
    ...
    for obj in objDirs:
        ...
        if os.path.exists(heightmap):
            addpath_3dmorph = "addpath('" + settings['path_run3dmorph'] + os.sep + "');"
            addpath_2dmorph = "addpath('" + settings['path_run2dmorph'] + "');"
            morph3dwrapper_settings = ["'" + run3dmorphPath+"'",
                                       "'" + morph3dPath + "'",
                                       "'" + focusedImageRGB + "'",
                                       "'" + heightmap + "'",
                                       "'" + obj + "'",
                                       "'" + settings['sampleID'] + "'",
                                       settings['macro_mode'].lower(),
                                       "'" + settings['unit'] + "'",
                                       settings['calibration'],
                                       settings['num_slices'],
                                       settings['zstep'],
                                       settings['kernel_size_OF'],
                                       settings['downsample_grid_size'],
                                       settings['savePDF'],
                                       settings['intensity_range_in'],
                                       settings['intensity_range_out'],
                                       settings['gamma'],
                                       settings['threshold_adjustment'],
                                       settings['noise_limit']
                                       ]

            morph3dwrapper = " morph3dwrapper(" + ','.join(morph3dwrapper_settings)
            exit = '); exit"'
            matlabCommand = 'matlab -nodisplay nodesktop -nosplash -r "'
            matlabCommand += ' '.join([addpath_3dmorph, addpath_2dmorph, morph3dwrapper, exit])

            # Run Matlab and suppress header
            os.system(matlabCommand + '| tail -n +13')
        else:
            print 'No heightmap found. Object skipped.'
            continue
    # Cleanup
...
if settings['savePDF'] == 'true' or settings['savePDF'] == '[]':
    ...
    for obj in objNamesClean:
        ...
        if os.path.exists(idtfFilePath):
            if 'linux' in sysArch:
                if commands.getstatusoutput("hash IDTFConverter.sh")[0] == 0:
                    idtfCommand = ' '.join([os.path.join(run3dmorphPath, 'mesh2pdf', 'bin', 'glx', 'IDTFConverter.sh'),
                                            ' -input', idtfFilePath, '-output', u3dOutputFilePath])
                elif commands.getstatusoutput("hash IDTFConverter")[0] == 0:
                    idtfCommand = 'IDTFConverter -input '+idtfFilePath+' -output '+u3dOutputFilePath
                else:
                    sys.exit('IDTFConverter executable not found, check PATH')
            elif 'darwin' in sysArch:
                idtfCommand = ' '.join(['cd', os.path.join(run3dmorphPath, 'mesh2pdf', 'bin', 'maci'), ';',
                                        './IDTFConverter', '-input', idtfFilePath, '-output', u3dOutputFilePath])
            commandFile.write(idtfCommand + ' | tail -n +30\n')
        else:
            print 'No idtf file found for obj: %s (Skipping)' % obj
            continue
    commandFile.close()
